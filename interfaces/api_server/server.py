...
executor = ThreadPoolExecutor(max_workers=max(1, int(config.api_max_workers)))
...
async def agent_query(payload: AgentQueryRequest):
    ...
    async with lock:
        try:
            ...
            response = await loop.run_in_executor(executor, agent.query, payload.question)
...
async def import_graph(payload: GraphImportRequest):
    ...
    async with lock:
        with KuzuManager(db_path=chat_db_path, config=config) as db:
            db.restore_graph_dump(payload.graph_dump)
